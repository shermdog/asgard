          ...
          <tr class="prop">
		      ...
		      <g:each var="tag" in="${tags}" status="i">
		        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
		          <td>${tag.key}<input type="hidden" class="tagName" value="${tag.key}"/></td>
		          <td><input type="text" id="tags.value.${tag.key}" name="tags.value.${tag.key}" value="${tag.value}" onchange="uniqueValue()"/></td>
